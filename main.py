...
with open('app.html') as fd:
    INDEX = fd.read()

async def handle_index(request):
    return web.Response(
        text = INDEX if request.remote != '127.0.0.1' else 'You should open this page on your phone instead.',
        content_type = 'text/html'
    )

Touch = namedtuple('Touch', ('slot', 'x', 'y'))
Button = namedtuple('Button', ('code', 'special'))
...
async def handle_init(request):
    ...
    buttons = tuple(Button(e[b], b.startswith('BTN_')) for b in await request.json())
    ...
    kb = UInput({
        e['EV_KEY']: tuple(button.code for button in buttons if not button.special),
    }, 'uremote-kb')
    tp = UInput({
        e['EV_KEY']: (e['BTN_TOUCH'], *TP_EVENTS, *(button.code for button in buttons if button.special)),
        e['EV_ABS']: (
...
async def handle_websocket(request):
    ...
    async for msg in sock:
        ...
        if t in (0, 1):
            button = buttons[p[0]]
            d = kb if not button.special else tp
            d.write(e['EV_KEY'], button.code, not t)
            d.syn()
        elif t in (2, 3, 4):
            _touches = tuple(
                Touch(
            ...
            if t != 3:
                if not touches:
                    tp.write(e['EV_KEY'], e['BTN_TOUCH'], True)

                touches += len(_touches) * (1 if t == 2 else -1)

                if not touches:
                    tp.write(e['EV_KEY'], e['BTN_TOUCH'], False)

                for i, ev in enumerate(TP_EVENTS):
                    tp.write(e['EV_KEY'], ev, i + 1 == touches)
